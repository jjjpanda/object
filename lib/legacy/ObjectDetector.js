        ...
        this.stream.on('data', (data, image) => {
            ...
            this.model.detect(imgTensor).then(results => {
                if(results.length > 0 || this.addToGhostResults(results)) {
                    ...
                    const postDetectionMessage = results.length > 0 ? "objects detected but nothing changed from last time" : "no objects detected"
                    this.emit('object', postDetectionMessage)
                    throw new Error(postDetectionMessage)
                }
            }).then(arrayOfBoxes => {
