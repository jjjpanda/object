...
module.exports = class FFMPEG extends EventEmitter {
    constructor(options){
        super()
        const {inputUrl, fps, differenceImagePath, pixelChangePercentTolerance, pixelThreshold} = options
        this.input =inputUrl ? inputUrl : "__nothing__"
        this.fps = fps ? fps : 1
        this.differenceImagePath = differenceImagePath
        this.pixelThreshold = pixelThreshold ? pixelThreshold : 0.1
        this.pixelChangePercentTolerance = pixelChangePercentTolerance ? pixelChangePercentTolerance : 0.01
    ...
    differentThanGhost() {
        ...
        if(current.length > 0 && previous.length > 0){
            Promise.all([
                ...
                const diff = Jimp.diff(image1, image2, this.pixelThreshold)
                this.emit('change', diff.percent)
                ...
                if(diff.percent > this.pixelChangePercentTolerance){
                    this.emit('data', current, image2)
                }
            }).catch(e => {
                console.log(e)
            })
